...
void TrafficLight::waitForGreen()
{
    ...
    while (_queue.receive() != TrafficLightPhase::green){std::this_thread::sleep_for(std::chrono::milliseconds(1));}
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    // init random variable for cycle duration
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> distr(4000, 6000);
    long cycleDuration = distr(gen);
